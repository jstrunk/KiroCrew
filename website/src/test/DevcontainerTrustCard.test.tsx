...
/** Fresh client per render: retries off so a failed query asserts immediately,
 *  and no cache carried between cases that share a project key. */
...
describe('DevcontainerTrustCard', () => {
  ...
  it('drops a preview that resolves after the project changed', async () => {
    ...
    const { rerender } = render(withClient(
      <DevcontainerTrustCard
        projectDir="/repo"
  ...
  it('does not mark a new project trusted when the previous grant resolves', async () => {
    // The mutation guard's real case: grant A, switch to B before A's onTrust
    // settles. A stale completion must not flip B into the granted state.
    //
    // Regression: the generation counter is only meaningful if something bumps
    // it. When the config read moved to useQuery, the effect that incremented it
    // went with it, so the ref sat at 0 and every comparison matched.
    let resolveTrust: () => void = () => {}
    const onTrust = vi.fn().mockImplementation(
      () => new Promise<void>((res) => { resolveTrust = res }),
    )
    const loadConfig = vi.fn().mockImplementation((project: string) =>
      Promise.resolve(
        config({ digest: project === '/repo' ? 'aaaaaaaaaaaaaaaa' : 'ffffffffffffffff' }),
      ),
    )
    const { rerender } = render(withClient(
      <DevcontainerTrustCard
    ...
    await waitFor(() => expect(screen.getByText('aaaaaaaaaaaa')).toBeInTheDocument())
    fireEvent.click(screen.getByRole('button', { name: /^trust$/i }))
    expect(onTrust).toHaveBeenCalled()

    // Switch projects while the grant is still in flight.
    rerender(withClient(
      <DevcontainerTrustCard
        projectDir="/other"
        onTrust={onTrust}
    ...
    // A's grant now completes. B must NOT render as trusted.
    resolveTrust()
    await waitFor(() => expect(screen.getByText('ffffffffffff')).toBeInTheDocument())
