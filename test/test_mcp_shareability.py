...
class TestNameCollisionDoesNotInheritEvidence:
    """A verdict is keyed by execution identity; a row is keyed by name.

    Two agents may define the same server name with different commands. The rows
    merge, so serving either stored measurement to the merged row would hand an
    unmeasured definition the other one's ``recommend_share`` — a recommendation
    to share a backend nobody probed.
    """

    @staticmethod
    def _write_cache(tmp_path: Path, keys: list[vc.CacheKey]) -> None:
        cache = vc.VerdictCache(vc.cache_path(tmp_path))
        for key in keys:
            cache.put(key, vc.CachedPreflight(True, False, (), 1.0))
        cache.flush()

    def test_one_identity_per_name_is_served(self, tmp_path, monkeypatch) -> None:
        from kiro_crew.dashboard.handlers import mcp as handler

        self._write_cache(tmp_path, [_key(server_name="srv", command_args_hash="cmdA")])
    ...
    def test_two_identities_under_one_name_read_as_unmeasured(
        ...
        self._write_cache(
            tmp_path,
            [
                _key(server_name="srv", command_args_hash="cmdA"),
                _key(server_name="srv", command_args_hash="cmdB"),
            ],
        )
        monkeypatch.setattr(handler, "records_dir", lambda *_a, **_k: tmp_path)
        ...
        assert "srv" not in preflights, "an ambiguous name inherited a measurement"
        verdict = assess(
            ShareEvidence(name="srv", probe_ok=True, capabilities={}, preflight_ran=False)
        )
        assert verdict.recommend_share is False
