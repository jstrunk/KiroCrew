...
Entries for servers no longer configured are pruned so the file does not grow once per config edit.
...
`mcp_gateway/evaluate.py` owns the one policy question the layers below refuse to answer: which servers are worth paying for. Only servers whose execution identity has no cached measurement are provoked, so the steady state is a file read and a newly installed MCP costs two spawns once.
...
## Seeding: at start, into config, once per server

`mcp_gateway/seed.py` turns verdicts into configuration. Three properties, each chosen so the operator is not surprised:
