...
class AcpRuntime:
    ...
    async def spawn(self) -> None:
        ...
        def _resolve_env_off_loop() -> None:
            # KRB5CCNAME resolution lstat/stats /tmp/krb5cc_<uid>, and for the
            # kiro-cli backend the CLI's own KIRO_API_KEY is re-injected from
            # the data home's .env (post-scrub Docker; see
            # config.loader.inject_kiro_cli_api_key) — a file read. Both are
            # blocking syscalls that must not run on the loop, bundled into ONE
            # thread hop. Guarded: the sandbox temp file is live, so a
            # cancellation here must not orphan it.
            resolve_krb5_ccname(env)
            if self._acp_backend != ACP_BACKEND_KAS:
                # circular import: config.loader reaches acp via session_handle
