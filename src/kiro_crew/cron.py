...
def _open_inside_nofollow(path: Path, root: Path) -> int | None:
    ...
    if os.open not in getattr(os, "supports_dir_fd", set()):
        return _open_inside_nofollow_no_dirfd(path, root, leaf_flags)

    root_fd: int | None = None
    cur: int | None = None
    try:
        # The root itself is product-created and not agent-writable; opening it
        # WITHOUT O_NOFOLLOW keeps a legitimately symlinked data home working,
        # which is the one link on this path the operator may have made.
...
def _open_inside_nofollow_no_dirfd(path: Path, root: Path, leaf_flags: int) -> int | None:
    """Fallback for platforms without ``dir_fd`` (Windows).

    Keeps the pre-round-15 shape: ``realpath`` containment plus ``O_NOFOLLOW``
    on the leaf. Residual, accepted here and only here: a symlinked
    intermediate directory is not refused when its target resolves back INSIDE
    the agents root — the comparison passes, so one agent's directory symlinked
    at another's still reads the victim's files, and an intermediate swap
    between the check and the open is likewise undetected. Closing either needs
    ``openat``. Confirmed empirically rather than assumed: the inside-root
    impersonation test fails on the Windows CI shard and is skipped there,
    which is what pins this as a platform limit instead of a silent hole.
    """
    try:
