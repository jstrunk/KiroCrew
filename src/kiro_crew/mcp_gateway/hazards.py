...
class HazardLedger:
    ...
    def flush(self) -> None:
        ...
        The flush runs off the event loop while ``record`` keeps running ON it, so
        an observation can land between building the payload and clearing the
        dirty flag. Clearing unconditionally would drop that observation
        permanently — the ledger would look clean while the file lacked the entry,
        and a hazard the gateway actually saw would never withdraw a
        recommendation. So the generation is captured with the payload and dirty
        is cleared only if nothing moved in between.
        """
        if not self._dirty:
            return
