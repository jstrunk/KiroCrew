...
def _atomic_write(path: Path, data: dict) -> None:
    ...
    directory's inherited permissions.  ``restrict_on_error="warn"`` keeps the
    ``.env`` load-time policy: a store write matters more than a failed
    permission edit.  On Windows the lockdown shells out to ``icacls``, so
    async callers must hand the whole write to a worker thread rather than
    call this on the event loop.  Other paths (the shared global file, agent
    files) keep the mode-preserving helper — their lifecycles are owned by
    other tools.
    """
